...
ts = TimeSeries(key=' IPSVS2OULY0RV66K',output_format='pandas')
count = -1
test_size = 10
...
def graphData(stock,MA1,MA2,interval):
	...
	scores = news2sentiment(stock_symbol=stock, api_key=api_key)

	# Make sure scores match the number of rows in data
	if len(scores) > len(data):
		scores = scores[-len(data):]  # take the last N scores
	elif len(scores) < len(data):
		# If somehow there are fewer scores, pad with 0 (neutral sentiment)
		scores = [0]*(len(data) - len(scores)) + scores

	# FIX: Convert all column names to strings to avoid sklearn error
